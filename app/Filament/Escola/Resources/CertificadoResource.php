...
use App\Filament\Escola\Resources\CertificadoResource\Pages;
...
class CertificadoResource extends Resource
{
    ...
    {
        // Apenas alunos aprovados da instituição atual (tenant)
        return parent::getEloquentQuery()
            ->with(['candidate', 'classes.institution', 'classes.courseMap.course', 'evaluations'])
            ->whereHas('classes', fn ($q) => $q->where('institution_id', Filament::getTenant()?->id))
        ...
        return $table
            ...
            ->filters([
                Tables\Filters\SelectFilter::make('cia')
                    ->label('CIA')
                    ->options(fn () => Student::whereHas('classes', fn ($q) => $q->where('institution_id', Filament::getTenant()?->id))
                        ->whereNotNull('cia')->distinct()->pluck('cia', 'cia')->toArray()),
                Tables\Filters\SelectFilter::make('platoon')
                    ->label('Pelotão')
                    ->options(fn () => Student::whereHas('classes', fn ($q) => $q->where('institution_id', Filament::getTenant()?->id))
                        ->whereNotNull('platoon')->distinct()->pluck('platoon', 'platoon')->toArray()),
                Tables\Filters\SelectFilter::make('section')
                    ->label('Secção')
                    ->options(fn () => Student::whereHas('classes', fn ($q) => $q->where('institution_id', Filament::getTenant()?->id))
                        ->whereNotNull('section')->distinct()->pluck('section', 'section')->toArray()),
                Tables\Filters\SelectFilter::make('class_id')
                    ->label('Turma')
                    ->options(fn () => StudentClass::where('institution_id', Filament::getTenant()?->id)->pluck('name', 'id')->toArray()),
            ])
            ->headerActions([])
            ->actions([
